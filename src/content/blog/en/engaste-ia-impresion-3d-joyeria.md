...
tags: ["stone setting", "jewelry", "AI", "3D printing", "CAD", "ERP", "gem traceability"]
date: 2026-05-05
language: "en"
seoKeyword: "jewelry stone setting AI 3D"
---
...
## 1. Types of setting and why they matter operationally

Before talking about technology, it is worth being clear about what we are talking about. The main types of setting in contemporary jewelry, ordered by operational complexity:

### 1.1 Prong setting

The most recognizable: four or six metal prongs hold a central stone. It is the classic engagement solitaire setting. It requires precision in cutting the prongs, in the symmetrical closure and in the subsequent polishing. An uneven prong makes the stone look crooked. A prong that is too short and the stone falls out over time.
...
The stones are aligned between two parallel metal walls. It is widely used in wedding bands with small stones. Critical: perfect alignment and leveling between stones.

### 1.5 Invisible setting

The stones are cut with notches in their girdle and held in a hidden metal grid. It is the most complex setting: any poorly cut stone ruins the entire piece. Reserved for high jewelry.

### 1.6 Micro-setting and melee setting

Stones less than 1 mm in diameter. **Impossible to handle without high-magnification loupe or binocular microscope**. This is where AI and robotics begin to have their first real impact, as we will see.

---

## 2. Where value is lost in traditional stone setting

Before talking about what changes with technology, it is worth understanding what breaks in the classic process:

- **Seat calculation errors**: the milling of the hole for the stone is done according to the theoretical size declared by the supplier. But real stones have tolerances. A seat that is too small forces the stone; too large, leaves play.
...
## 3. Parametric CAD: setting starts before the piece
...
- **Simulate the assembly** and verify that there are no collisions, that light passes under the stone (what they call *correct faceting*), and that the proportions are aesthetically balanced.
- **Version the model**: the same reference with different types of setting as parametric variants.

**The critical point for a jewelry ERP:** each CAD model has an associated **bill of materials** that includes not only the weight of gold but also the type, number and exact specifications of each stone. When the manufacturing order is launched, the ERP reserves the material and the specific stones from the warehouse. Traceability begins here, not in the workshop.
...
- **DLP (Digital Light Processing)**: the most used in jewelry due to speed and resolution.
- **SLA (Stereolithography)**: high precision, ideal for pieces with fine detail.
- **Direct Wax Printing**: prints directly in castable wax compatible with microfusion.
...
- The seat **comes already made** from casting, no need to mill it manually.
- The **prongs come preformed** with the correct height and angle.
- The setter receives a piece that only needs to **place the stone and close**, saving between 30 and 60% of the time of each setting depending on complexity.

### 4.4 Stone-in-place: setting during casting

A technique that is growing rapidly: stones (typically natural or synthetic diamonds, sapphires, rubies, which withstand high temperatures) are placed **directly in the wax or resin model** before casting. The metal melts around the stone and, when cooled, remains set without need for subsequent processing.

**Saving:** a piece with 50 traditionally set melee takes 4-5 hours. With stone-in-place, **the same piece comes out of the cylinder with all the stones already in place**.

**Limitations:** it only works with thermally stable stones. Emerald, opal, turquoise, amber, pearl and practically any organic gem **cannot withstand the process**.

**What the ERP must control:** complete traceability of stones from supplier to setting in a specific piece, with batch number, certificate (Kimberley for natural diamonds, origin certification for synthetics) and responsible operator.

---

## 5. Computer vision: quality control that doesn't get tired

This is where AI has had its most immediate and measurable impact on stone setting. Three real applications in jewelry workshops 2026:

### 5.1 Post-setting inspection by computer vision

High-resolution macro cameras capture the piece from multiple angles. Deep learning models (typically Vision Transformers trained with thousands of labeled images) detect:
...
### 5.2 Automatic gem grading

Systems such as **Gemtelligence** and **Digital Gemmology 3.0** use computer vision to evaluate the 4 Cs (color, clarity, cut, weight) of diamonds. Published data:

- **97% accuracy** identifying natural diamonds across more than 10,000 samples.
...
A more subtle but very valuable application: AI can determine if an **internal inclusion of the diamond will be visible once set in a specific mounting**. Before, you had to trust the expert's eye. Now you can simulate and choose the most economical "eye-clean" diamond for each type of setting, saving money for the end customer without losing apparent visual quality.

---

## 6. Robotics: micro-setting with micron precision

This is more in an emerging phase than consolidated in 2026, but it is worth mentioning: **AI-assisted robotic arms** are being tested in high-volume jewelry workshops to place **melee diamonds with micron precision**.

The typical process:

1. The cast piece is placed on a calibrated coordinate table.
2. A camera identifies the seats with computer vision.
3. A robotic arm with precision tweezers picks up each melee from a classified feeder.
...
It is the direction in which the sector is going, but **the current cost of equipment (jewelry-precision robotic arms, calibrated vision systems, melee feeders)** limits it for now to very large manufacturers. In independent and medium jewelry, the most profitable combination is still **CAD + 3D printing + post-setting computer vision + human setter**.

---

## 7. Summary table: stone setting before and after
...
- **Aesthetic judgment**. Deciding if a prong "looks good" on a unique piece is not a computable problem. It is judgment.
- **Adaptation to the specific piece**. Each gem is slightly different. The setter feels the piece, the machine follows a sheet.
- **High jewelry settings and unique pieces**. Invisible setting, irregular stones, restorations, commemorative pieces: all are human territory for a long time.
...
Detailed inventory of each stone, not just quantities. For each batch: type, cut, exact measured size, weight in carats, color, clarity, certificate (Kimberley for natural diamonds, origin certification for synthetics), supplier and cost.

### 9.2 Automatic reservation when launching order

When launching a manufacturing order with setting, the ERP **automatically reserves the specific stones** from the warehouse that meet the CAD model specifications. Avoids two orders competing for the same batch.

### 9.3 Piece-by-piece traceability

Each finished piece has the batch number of each set gem associated with it. If the customer claims within warranty, you can know **exactly** which gem it has, from which supplier, with which certificate.
...
The small stones lost during setting are real loss. Measuring it by operator, by type of piece and by phase is the first step to reducing it.

### 9.6 Quality control images

Each piece with valuable setting must have **post-setting inspection images** archived. It is proof of quality, warranty support and, if done with computer vision, feeds the model to continuously improve detection.

---

## 10. Conclusion: stone setting remains art, but art benefits from good engineering

For a long time the jewelry sector operated under a false dichotomy: either you were a "pure" artisan workshop where the trade was sacred, or you were an industrial factory where quality was sacrificed for volume. **That dichotomy died in 2026**.
...
**Did you find this article useful?** Share it with your workshop team or subscribe to the RayTecno blog to receive a monthly strategic analysis.
